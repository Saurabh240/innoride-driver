...
class AddNewDepositController extends GetxController {
  ...
  setPaymentMethod(AppPaymentGateway? method) {
    ...
    depositLimit =
        '${StringConverter.formatNumber(method?.minAmount?.toString() ?? '-1')} - ${StringConverter.formatNumber(method?.maxAmount?.toString() ?? '-1')} $currency';
    ...
    if (responseModel.statusCode == 200) {
      ...
      if (insertResponseModel.status.toString().toLowerCase() == "success") {
        ...
        CustomSnackBar.error(
            errorList:
                insertResponseModel.message ?? [MyStrings.somethingWentWrong]);
      }
    } else {
      CustomSnackBar.error(
