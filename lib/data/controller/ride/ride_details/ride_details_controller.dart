...
class RideDetailsController extends GetxController {
      ...
      if (responseModel.statusCode == 200) {
        ...
        if (model.status == MyStrings.success) {
          ...
          if (tRide != null) {
            ...
            pickupLatLng = LatLng(
                StringConverter.formatDouble(tRide.pickupLatitude.toString()),
                StringConverter.formatDouble(tRide.pickupLongitude.toString()));
          ...
          mapController.loadMap(
              pickup: pickupLatLng, destination: destinationLatLng);
  ...
  Future<void> startRide(String rideId) async {
    ...
    try {
      ResponseModel responseModel =
          await repo.startRide(id: ride.id ?? '-1', otp: otpController.text);
      if (responseModel.statusCode == 200) {
  ...
  Future<void> updateLocation() async {
    try {
      ResponseModel responseModel =
          await repo.updateLocation(id: ride.id ?? '');
      if (responseModel.statusCode == 200) {
        AuthorizationResponseModel model = AuthorizationResponseModel.fromJson(
            jsonDecode(responseModel.responseJson));
        if (model.status == MyStrings.success) {
