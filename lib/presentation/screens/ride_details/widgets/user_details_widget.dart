...
class UserDetailsWidget extends StatelessWidget {
  ...
  const UserDetailsWidget(
      {super.key, required this.ride, required this.imageUrl});
          ...
          child: GestureDetector(
            onTap: () {
              Get.toNamed(RouteHelper.userReviewScreen,
                  arguments: ride.user?.id.toString());
            },
        ...
        Expanded(
          ...
          child: ride.status == "1"
              ? SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ride.rideType == AppStatus.RIDE_TYPE_INTERCITY
                          ? SvgPicture.asset(MyIcons.intercityCar)
                          : Image.asset(MyImages.city, width: 30, height: 30),
                      const SizedBox(width: Dimensions.space10),
                      CardColumn(
                        header: ride.rideType == AppStatus.RIDE_TYPE_INTERCITY
                            ? MyStrings.interCityRide
                            : MyStrings.cityRide,
                        body: ride.service?.name ?? '',
                        headerTextStyle: boldExtraLarge.copyWith(
                            color: MyColor.primaryColor),
                        bodyTextStyle: regularMediumLarge.copyWith(
              ...
              : Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          Get.toNamed(RouteHelper.rideMessageScreen,
                              arguments: ride.id.toString());
                        },
                        child:
                            const CustomSvgPicture(image: MyIcons.messageIcon),
                      ),
                    ],
                  ),
                ),
